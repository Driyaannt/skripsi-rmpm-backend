...
const trailLog = db.skripsiIncomingRmpm.trailsLogs;
...
module.exports = (req, res, data) => {
    ...
    let userCode = req.headers['user-token'] ? (decrypt(req.headers['user-token']) == '' ? req.headers['user-token'] : decrypt(req.headers['user-token'])) : '';

    let trail_logs = {
        endpoint: req.originalUrl,
        ip: req.ip.substr(7, req.ip.length),
        request: request.length > 300 ? request.substring(0, 300) : '',
        response: response.length > 300 ? response.substring(0, 300) : '',
        datetime: req.customDate,
        method: req.method,
        url: req.headers['url-access'],
        ip_address: req.headers['ip_address'],
        user: userCode,
        status: data.status,
    };

    console.log("IPPPPP", req.ip);

    // if env value return true log into file
    if (process.env.APP_LOG == "true") {
        // resLog(req, msg);
        // console.log("ini kekrdwaddddddddddddddddddddddddddddddddddddddin", trail_logs);
        trailLog.create(trail_logs)
    }
